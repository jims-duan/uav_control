...
private:
    ...
    // 记录通道10（索引9）上次的状态，初始值为1500（中立位置）
...
public:
    ...
    void rcCallback(const mavros_msgs::RCIn::ConstPtr& msg)
    {
        ...
        // 通道10（索引9）逻辑 - 当值为1948时发布点，值为1048时停止
        // 注意：channels[9] 对应遥控器的通道10（因为数组从0开始）
        int ch10_value = msg->channels[10];
        
        if(ch10_value == 1946 && last_channel_10_state != 1946)
        {
            ROS_INFO("通道10 = 1948，发布导航点到Rviz");
            publishMoveBaseGoal();  // 切换到1948时发布一次点
        }
        else if(ch10_value == 1048 && last_channel_10_state != 1048)
        {
            ROS_INFO("通道10 = 1048，停止发布点");
            // 这里可以添加停止导航的逻辑（如果需要）
        ...
        last_channel_10_state = msg->channels[10];
    }
